    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between mt-6 md:mt-8 gap-5 md:gap-6 relative z-10">
        ...
        <div className="flex gap-2 md:gap-3 flex-wrap justify-center md:justify-end w-full md:w-auto" onClick={(e) => e.stopPropagation()}>
          ...
          d<SocialBtn type="bilibili" url={siteConfig.social?.bilibili} />
        </div>
      </div>
    </div>
  );
...
function SocialBtn({ type, url, onClick }: { type: string, url?: string, onClick?: () => void }) {
  const getIcon = () => {
    switch (type) {
      ...
      case 'bilibili': return <svg className="w-4 h-4 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.813 4.653h.854c1.51.053 2.77.558 3.795 1.514a5.56 5.56 0 0 1 1.498 3.651c.073.731.074 1.453.074 2.18v1.724c-.008.743-.02 1.484-.146 2.217-.353 2.067-1.55 3.588-3.362 4.49-.46.229-.946.407-1.444.548-.47.134-.94.23-1.417.31-.72.12-1.446.141-2.174.146a35.583 35.583 0 0 1-5.658-.039c-.738-.066-1.472-.186-2.196-.337-.44-.092-.877-.205-1.305-.345-.637-.208-1.237-.489-1.802-.846a7.297 7.297 0 0 1-1.655-1.402 8.123 8.123 0 0 1-1.456-2.316 9.554 9.554 0 0 1-.544-2.09c-.084-.535-.096-1.08-.103-1.622V9.92c0-.77.054-1.537.234-2.283a6.024 6.024 0 0 1 1.56-2.884 5.595 5.595 0 0 1 3.8-1.514h.856l1.014-1.775c.293-.483.96-.92 1.54-.519.245.17.453.385.624.635l.789 1.66h1.626l.79-1.66c.171-.25.379-.465.623-.635.58-.401 1.247.036 1.54.52l1.014 1.774zm-8.624 4.217a1.32 1.32 0 0 0-1.298-1.319 1.32 1.32 0 0 0-1.299 1.32 1.32 1.32 0 0 0 1.3 1.319 1.32 1.32 0 0 0 1.297-1.32zm8.166 0a1.32 1.32 0 0 0-1.298-1.319 1.32 1.32 0 0 0-1.299 1.32 1.32 1.32 0 0 0 1.3 1.319 1.32 1.32 0 0 0 1.297-1.32z"/></svg>;
      default: return null;
    }
  };
